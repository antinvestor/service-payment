...
// Package credentials resolves pawaPay API credentials from the settings
...
// ErrMissingAPIToken indicates no pawaPay API token could be resolved.
var ErrMissingAPIToken = errors.New("missing required pawaPay credentials (api_token)")

// Resolver resolves pawaPay credentials with a 3-level fallback:
